...
function updateTimeDisplay()
{
 if (parent.videoframe.videoSet==true && parent.videoframe.selectedAVSource.type<30)
  setElementHTML("videotime", getString("e_videopos")+" : "+getTimeString(parent.videoframe.selectedAVSource.getPosition()));
 else
...
function setSlideTime(index, pos)
{
 ...
 if (r>0)
 {
  ...
  element.value=getTimeString(parent.videoframe.times[parent.videoframe.avLang][index]);
  if (r==1)
...
function updateSlideTime()
{
 ...
 else
 {
  ...
  element.value=getTimeString(parent.videoframe.times[parent.videoframe.avLang][parent.videoframe.currentSlide-1]);
 }
}
...
function setSubtitleTime(index, pos)
{
 ...
 if (r>0)
 {
  ...
  element.value=getTimeString(parent.videoframe.subTimes[parent.videoframe.avLang][index]);
  if (r==1)
...
function updateSubtitleTime()
{
 ...
 else
 {
  ...
  element.value=getTimeString(parent.videoframe.subTimes[parent.videoframe.subtitleLang][parent.videoframe.currentSubtitle-1]);
 }
}
...
function processTimeString(data)
{
 ...
 if (semi>dot)
 {
  ss=parseFloat("0."+data.substring(semi+1, data.length));
  alert(ss);
 }

...
function setLiveBroadcastHTML(name)
{
 ...
 var cfg="maxKbps="+broadcastMaxKbps+"&server="+parent.videoframe.flashServer+"&stream="+name+"&host="+parent.videoframe.flashHost+"&slides=";
 if (parent.videoframe.slideSet)
  for (var loop=0; loop<parent.videoframe.titles[parent.videoframe.slideLang].length; loop++)
   cfg=cfg+parent.videoframe.titles[parent.videoframe.slideLang][loop].replace('&', ' ')+"{^}";
 else
   cfg=cfg+"----------";

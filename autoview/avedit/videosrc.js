...
function submitVidSource()
{
 if (checkType())
 {
  ...
  var flvi=url.indexOf(".flv");
  //var flashDiri=url.indexOf(opener.flashDir);
  if (flvi>-1 && url.indexOf("rtmp://")<0 && opener.liveCaptureInstalled) // && flashDiri>-1)
  {
   if (opener.alwaysFlashStream==true || confirm(opener.getString("e_streamquestion")))
   {
    //document.form.url.value=opener.parent.videoframe.flashServer+"/"+document.form.url.value.substring(flashDiri+opener.flashDir.length+1, flvi);

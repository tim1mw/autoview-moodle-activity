...
{
 var data=getTimeList(parent.videoframe.times[parent.videoframe.avLang], parent.videoframe.titles[parent.videoframe.avLang].length,
...
function updateTimeDisplay()
{
 if (parent.videoframe.videoSet==true && parent.videoframe.selectedAVSource.type<30)
  setElementHTML("videotime", getString("e_videopos")+" : "+getTimeString(parent.videoframe.selectedAVSource.getPosition()/100));
 else
...
function setSlideTime(index, pos)
{
 ...
 if (r>0)
 {
  ...
  element.value=getTimeString(parent.videoframe.times[parent.videoframe.avLang][index]/100);
  if (r==1)
...
function updateSlideTime()
{
 ...
 else
 {
  ...
  element.value=getTimeString(parent.videoframe.times[parent.videoframe.avLang][parent.videoframe.currentSlide-1]/100);
 }
}
...
function setSubtitleTime(index, pos)
{
 ...
 if (r>0)
 {
  ...
  element.value=getTimeString(parent.videoframe.subTimes[parent.videoframe.avLang][index]/100);
  if (r==1)
...
function updateSubtitleTime()
{
 ...
 else
 {
  ...
  element.value=getTimeString(parent.videoframe.subTimes[parent.videoframe.subtitleLang][parent.videoframe.currentSubtitle-1]/100);
 }
}
...
function convertSlides()
{
 ...
 if (conversionURL.length>0)
  data=data+" <table align=\"center\">\n"+
   "  <tr>\n"+
   "   <td>"+getString("e_slide_file")+"<br />(.pdf/.ppt/.odp/.sxi)</td>\n"+
...
function setLiveBroadcastClient()
{
 ...
 if (name==null)
 {
  ...
  if (typeof(parent.videoframe.selectedLang)!="undefined")
   ln=parent.videoframe.selectedLang;
  parent.videoframe.addUpdateAVSrc(null, -1, ln, parent.videoframe.VIDEO_FLASHBROADCAST,parent.videoframe.flashServer+"/"+name, null, null);
 }
 else
...
function setLiveBroadcastHTML(name)
{
 ...
 var cfg="server="+parent.videoframe.flashServer+"&stream="+name+"&host="+parent.videoframe.flashHost+"&slides=";
 if (parent.videoframe.slideSet)
  for (var loop=0; loop<parent.videoframe.titles[parent.videoframe.slideLang].length; loop++)
   cfg=cfg+parent.videoframe.titles[parent.videoframe.slideLang][loop]+"{^}";
 else
   cfg=cfg+"----------";
